...
from .models import Item, Sub, Main, Category
import json
...
def index(request):
	...
	if request_j["type"] == "home":
		region_si = request_j["region_si"]
		response_j = dict(data=list())
		i = 0
		for data in request_j["data"]:# data : {"sub_id":"91"}
	...
	if request_j["type"] == "result":
		...
		for i in range(5):
			...
			items = Item.objects.filter(category__sub__id=request_j["data"]["sub_id"], region__name=request_j["data"]["region_si"], price_date=dateutil.parser.parse(now))
			for item in items:
				price_date = str(item.price_date)
				price_date = price_date[0:4]+price_date[5:7]+price_date[8:]
				response_j["data"].append(dict(grade=item.category.name, price_r=item.price_r, price_w=item.price_w, unit_r=item.unit_r, unit_w=item.unit_w, price_date=price_date))
		return HttpResponse(json.dumps(response_j), content_type="application/json")
